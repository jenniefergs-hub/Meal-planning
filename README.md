...
- **Receipt parsing is heuristic.** Grocery receipt emails/PDFs have no standard format, so the parser looks for common patterns (quantity/name/price lines, "delivered/ordered" style UK order receipts, HTML table rows), handles £/$/€ prices, and flags blacklist words (tax, tip, delivery, charge, total, etc.) as whole words so they don't false-positive inside a product name (e.g. "coffee" containing "fee"). It won't be perfect for every retailer's format — that's why parsed items always go through the review queue before becoming pantry ingredients.
